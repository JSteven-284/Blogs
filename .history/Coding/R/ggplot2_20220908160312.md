...
6. Statistic Transformation
   ...
   Here, the y variable could be replaced by after_stat(prop) or something like to add another stat manually, because the original default one has been cancelled with "identity", which menas do nothing for the data.
