...
6. Statistic Transformation
   ...
   Here, the y variable could also be replaced by after_stat(prop/count/) or something like to add another stat manually, because the original default one has been cancelled with "identity", which menas do nothing for the data.
