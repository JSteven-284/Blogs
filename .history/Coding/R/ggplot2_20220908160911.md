...
6. Statistic Transformation
   ...
   Here, the y variable could also be replaced by after_stat(prop/count/...) or something else to add another stat manually based on x, because the original default one has been cancelled with "identity", which menas do nothing for the data.
